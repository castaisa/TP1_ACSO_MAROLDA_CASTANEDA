...
void implement_CBZ(instruction instruct) {
    ...
    if (zero == 1) {
        ...
        if (instruct.br_address & (1 << 18)) {
            // Si el bit 18 está encendido, extendemos el signo para convertirlo en un número negativo
            signed_offset = (int64_t)(instruct.br_address | 0xFFFFFFFFFFFC0000);
        } else {
            // Si el bit 18 no está encendido, mantenemos los bits originales
            signed_offset = (int64_t)(instruct.br_address & 0x3FFFF);
        }

        // Actualizar el PC (Program Counter) con el desplazamiento calculado
